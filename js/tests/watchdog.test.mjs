// Behavioural probe for the two standing watchdogs — the `watchdog` and
// `watchdog-exit` subcommands of the single shipped file
// skills/standing/scripts/iskron.mjs (graph nks-dev: #4058, #4059). They have no
...
//
// ISKRON_WATCHDOG_DIR points the same probe at any directory holding an
// iskron.mjs (a built bundle, an installed one, a previous revision) so it can
// be shown red before a fix.
//
...
const DIR =
  process.env.ISKRON_WATCHDOG_DIR || join(HERE, "..", "..", "skills", "standing", "scripts");
const FILE = join(DIR, "iskron.mjs");
...
function runWatchdog(sub, url, env, timeoutMs = 5_000) {
  const proc = spawn(process.execPath, [FILE, sub, url], {
    env: { ...process.env, ...env },
  ...
  return new Promise((resolve) => {
    const t = setTimeout(() => {
      proc.kill("SIGKILL");
      resolve({ exit: null, out, err });
    }, timeoutMs);
    proc.once("exit", (code) => {
      clearTimeout(t);
      resolve({ exit: code, out, err });
    });
  });
}
...
for (const file of ["watchdog", "watchdog-exit"]) {
  test(`${file}: a dead token is a loud non-zero exit, even with the busy-line publisher on`, async () => {
    ...
    try {
      ...
      assert.notEqual(
        r.exit,
        null,
        `${file} is still alive 5s after a 4000 close — a dead token that looks like an empty inbox`,
      );
      assert.notEqual(
        r.exit,
        0,
        `${file} exited 0 on a dead token — indistinguishable from a clean stop`,
      );
      assert.match(
        r.out + r.err,
        /токен мёртв/,
        "the last line must name the dead token for the doer",
      );
    } finally {
      await srv.close();
    }
  ...
  test(`${file}: drops against a live service also end loudly, never in silence`, async () => {
    ...
    try {
      ...
      assert.notEqual(
        r.exit,
        null,
        `${file} kept running after three fast drops with the service up — it should have handed the question to the doer`,
      );
      assert.notEqual(
        r.exit,
        0,
        `${file} exited 0 there: the doer reads a clean stop and keeps believing the socket is held`,
      );
      assert.match(
        r.out + r.err,
        /спроси о токене/,
        "the last line must hand the doer the question it cannot answer itself",
      );
    } finally {
      await srv.close();
    }
