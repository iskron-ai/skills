  ...
  if (resumed) {
    ...
    how = listensElsewhere
      ? wasEvicted(realm, karta, name)
        ...
        : predecessorDead
          ? "слушающим доска ещё читает прежний мост этого каталога, а он мёртв (его сокет не отвечает, запись держания цела) — только register; доска отпустит его по окну слуха контура (listening_window_seconds в hello; без него — около минуты), и тот же вызов вернёт место с диска тем же адресом — повтори"
