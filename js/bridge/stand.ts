  ...
  if (resumed) {
    ...
    how = listensElsewhere
      ? wasEvicted(realm, karta, name)
        ...
        : predecessorDead
          ? "слушающим доска ещё читает прежний мост этого каталога, а он мёртв (его сокет не отвечает, запись держания цела) — только register; доска отпустит его по окну слуха контура — его называет hello полем listening_window_seconds (нет поля — сервер окна не сообщает; до этого поля наблюдалось около минуты), и тот же вызов вернёт место с диска тем же адресом — повтори"
