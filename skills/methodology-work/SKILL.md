...
1. iskron_orient(realm="methodology", include_schema=false)
   → Прочти корневые контуры (元意行德忠忘), entry/exit-крии, ключевые узлы, зовущие вимарши, натяжения
...
3. Прочти карту: секция ACTIVE BIANHUA в orient (lens="bianhua" — весь лес) —
   зовущая работа живёт там anga-вимаршами. Сид-вимарша, если есть, — лишь
...
ДЕЛАЙ:
  ...
  3. Не покрыто → создай феномен given_as=grundsatz. Модусы — по тому, чья воля и какое свидетельство за ним
     ...
     — воля: твоё предложение или усмотренный тобой узор → chanda (предложен, жив, не в силе);
       ...
       кто сказал и его слово — в reasoning, содержание — в теле; то, что держим не мы
...
КРИТИЧЕСКИЙ ПАТТЕРН — агент пытается заглушить натяжение:
  ...
  Видишь атрибут, поставленный ради глушения натяжения, → это неверно. Запиши антипаттерн.
```
...
- Зовущая работа живёт на карте: вимарши, прицепленные `anga` к превращениям, которые они двигают.
