...
//
// И он же несёт слово делателя НАРУЖУ — строку занятости, см. вторую половину
// файла: секрет, которым она удостоверяется, есть у сторожа и нет у рабочей
// сессии, так что публиковать её больше некому.
//
...
// Строку занятости удостоверяет слушающий секрет, а он у СТОРОЖА — у рабочей
// сессии его нет, и платформа будит делателя поправить строку, которую поправить
// нечем. Поэтому сторож несёт слово делателя наружу так же, как несёт кадры
// внутрь: делатель пишет ТЕКСТ в файл, сторож публикует его изменения. Секрет при
// этом не касается диска — делатель пишет только слова, — а авторство не слабеет:
// писать в файл может лишь тот, кто сторожа и запустил. Без переменной половина
// не включается, и старое поведение сохраняется дословно.
const sayFile = process.env.ISKRON_CHANNEL_SAY;
// Статусный адрес: ВЫДАННЫЙ рядом с сокетом — первый; вывод из формы — фолбэк,
// потому что сторожу вручают сокет, а не ответ connect целиком. Форма дрейфует,
// и выданная строка всегда права — задавай ISKRON_CHANNEL_STATUS, когда он у тебя есть.
