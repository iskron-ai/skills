...
Сервер удалённый (streamable HTTP + OAuth); из него на машину пользователя не ставится ничего. Ставится эта поставка — и она несёт собственный транспорт: `scripts/iskron.mjs`, рядом с этим файлом — один файл, несущий мост, обоих сторожей стояния и диагностику `doctor`. Записей MCP-сервера поставка несёт две: в плагине Claude Code — **stdio-мост из самой поставки** (`.mcp.json` с путём к этому файлу; ступень 0 ниже), в архиве claude.ai и манифесте Codex — **нативная http-запись**, потому что запускать локальный процесс умеет не всякий харнесс, а путь к файлу внутри установленной поставки у каждого свой. Где записи моста нет, мост ставишь ты, и ниже сказано как.
...
0. **Плагин Claude Code уже поднял мост.** Плагинная запись `.mcp.json` — сам мост из поставки: `claude mcp list` показывает `plugin:iskron:iskron` stdio-записью, первый вызов тула отвечает ошибкой с адресом авторизации, и после клика в браузере тулы отвечают. Копировать ничего не нужно, обновление приезжает с плагином (новая сессия или `/reload-plugins`). «Поднятие моста» ниже — для харнесов без плагинной записи и для второго инстанса; `doctor` тем же файлом говорит, какая сборка стоит и работает ли она.
