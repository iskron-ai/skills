...
Каждая синтетическая ошибка моста и каждая строка `grant.log` несут сборку вида `v0.2.0+a1b2c3d4` — версию и хеш реально исполняемого файла. В любом репорте о проблеме первым делом цитируй её: она датирует код точнее любых слов, и «известный починенный дефект на старой редакции» отличается от «нового дефекта» одним взглядом.
