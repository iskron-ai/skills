...
1. **На машине есть Node ≥22, и конфиг харнесса берёт `command` + `args`** (stdio-сервер) → «Поднятие моста» ниже. Это путь по умолчанию, и единственный, который держит привязку стояния.
