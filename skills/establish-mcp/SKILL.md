...
Каждый другой скилл этой поставки предполагает, что MCP-тулы `iskron_*` уже в сессии и идут через мост. Этот скилл ставит транспорт — и чинит его, когда тулов нет или они есть по имени и отказывают в деле. Его работа кончается там, где начинается работа entry: сессией, чей первый вызов `iskron_orient` отвечает.

Сервер удалённый (streamable HTTP + OAuth); из него на машину пользователя не ставится ничего. Ставится эта поставка — и она несёт собственный транспорт: `scripts/iskron-bridge.mjs`, рядом с этим файлом.

## Сначала: которым путём

Путь по умолчанию — мост, и он не запасной. У него две обязанности, которых у нативного MCP-клиента харнесса нет: держать грант без походов в браузер (обновление в простое, один флоу на всю машину, логин тратится последним) и держать **привязку стояния** через пересборку сессии — заголовок стояния на открытии сессии и переигранный `register`, без которых записи агента уходят без автора. И отказы у него всегда громкие: каждая ошибка называет `iskron-bridge`, сборку и вердикт. Нативная http+OAuth регистрация — запасной путь ровно на случай, когда мост не поднять.

1. **Claude Code с плагином iskron** → делать нечего: `.mcp.json` плагина уже запускает мост из корня установленного плагина (`${CLAUDE_PLUGIN_ROOT}`) — копия не нужна, редакция моста едет с плагином и обновляется вместе с ним. Первый вызов откроет браузер. Убедись, что тулы идут через мост, а не через параллельную нативную запись: ошибка моста называет `iskron-bridge` и сборку.
2. **Любой харнесс, чей конфиг берёт `command` + `args`** (stdio-сервер), и Node ≥20 на машине → «Поднятие моста» ниже.
3. **Мост не поднять** — нет Node ≥20, харнесс не запускает локальных процессов (веб-харнесс, claude.ai) или не держит stdio-сервер → нативная удалённая запись (`url` / `httpUrl` / `serverUrl`, `type: "http"` / `"remote"` / `"streamableHttp"`) в пользовательском скоупе плюс глагол авторизации харнесса (`/mcp`, `codex mcp login`, `opencode mcp auth`, кнопка Authenticate, UI коннекторов). Цена названа вслух: привязку стояния через пересборку сессии здесь не держит никто, и standing велит `register` перед каждой несущей записью.
4. **Удалённая запись есть, но глагола авторизации нет нигде** — доки показывают ручные заголовки `Authorization` → OAuth нативно не случится; мост или ничего.
...
На Claude Code с плагином этот раздел не нужен — мост уже бежит из корня плагина. Для остальных харнессов:
...
3. **Зарегистрируй мост обычным stdio-MCP-сервером — в ПОЛЬЗОВАТЕЛЬСКОМ скоупе, не проектном.** Граф — собственный контур пользователя и следует за ним через каждый репозиторий; проектно-скоупная запись теряет его в первой же соседней директории. Потому регистрация идёт в пользовательский конфиг харнесса (файл в домашней директории), никогда не в пер-проектный — проектный скоуп только по явной просьбе пользователя. Общая форма:
   ...
   (скоуп по умолчанию — проектно-локальный: передай `--scope user` явно; на Claude Code это нужно только без плагина — плагин несёт запись сам). URL сервера добавляй ещё одним аргументом только для недефолтного инстанса. В JSON-конфигах, не раскрывающих `~`, пиши абсолютный домашний путь.
