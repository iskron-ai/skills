...
| P4 | Натяжения всегда правдивы | Никогда не подавляй. Всегда замыкай структуру. Натяжение кажется неправым → баг детектора, а не повод его заглушить. |
...
ДЕЛАЙ:
  ...
  2. Каждый hint: объём работы, критерии приёмки — «Answered when» указывает наблюдение
     ...
     **стрелка `posed_to`** (вимарша→роль)
     ...
     chanda→adhimoksha — никогда атрибутом срочности; блокирующесть — свойство пары),
...
| веер — не имя детектора, а канон: много связей одного рода на узле | грубейших связей больше, чем вмещает экран внимания; сегодня линза зовёт счётом деяний без объемлющего — тот же зов; счёт стрелок сам по себе не долг | Не «раздели» — назови среднее со своей грубой связью: зонт применяет, ест или даёт целое либо род, листья уточняют; зонт без связи — папка (**writing**, Решение 5; среднее по роду веера — **assembly**, шаг 3) |
