...
name: iskronify
slash: true
description: "Используй, когда просят прокачать репозиторий — iskronify: завести или освежить AGENTS.md / CLAUDE.md по стандарту iskron, прошить ритуалы сессии (orient на старте, push→NKS-хуки, гейт качества, memory-guard). Триггеры: iskronify, «прокачай репо», «привести проект к стандарту», «завести/обновить AGENTS.md», set up AGENTS.md, «наведи порядок в конфиге агента». AGENTS.md — производный вид: каждая забота аудируется против источника истины и ре-проецируется при устаревании, авторское суждение сохраняется. Нужны тулы iskron_* для NKS-шагов."
---
...
**Контракт: `2`.** Шаг 7 штампует это число в каждый записанный `AGENTS.md`. Увеличивай на единицу только когда перемена здесь или в скелете делает уже сгенерированный файл *неверным* — секция добавлена, переименована или изъята, ритуал изменён, имя тула выпало; никогда — ради формулировок. Репо с меньшим штампом получает полную дугу, и ты называешь контракт, из которого пришёл его конфиг.
...
- **Проштампуй контракт.** Хвостовая строка `AGENTS.md`: `*(iskronify: contract <число из шапки этого файла> — re-run when the installed contract is higher, or when the sources this file derives from have moved since.)*` На рефреше перезапиши старый штамп; никогда не оставляй два. Конфиг **без** штампа — или со штампом-**датой** из схемы, которую этот счётчик сменил, — предшествует контракту: считай его ниже любого числа и прогони полную дугу.
