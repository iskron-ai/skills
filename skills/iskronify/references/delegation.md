...
- Рабочую копию не трогай — она общая с автором и одна на машине: ни одной git-команды, пишущей в рабочее дерево, индекс или ссылки (`checkout`, `switch`, `restore`, `reset`, `stash`, `clean`, `apply`, `cherry-pick`, `worktree add` — примеры, не перечень); дифф и файлы ветки читай через `git diff origin/main...HEAD` и `git show <ref>:<путь>`. Изолирован ли ты — проверь, не предполагай: в отдельном worktree `git rev-parse --git-common-dir` отличается от `git rev-parse --git-dir`; совпадают — ты в общей копии, и запрет выше — единственная ограда.
