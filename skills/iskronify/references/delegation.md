...
- Поверхность тулов графа суди по старшинству того, что каждый источник покрывает: снимок `fixtures/surface.json` снят с сервера своими руками и сверяет имена тулов и глобальные словари — его срабатывание на мёртвом имени старше любого текста; параметры вызова и пер-тульные действия снимок несёт словарями, но не сверяет — о них старше всего описание самого тула (`action="?"`, текст описания в сессии), а код моста, который лишь ретранслирует вызов, — младше обоих. Параметр, названный описанием тула, жив, что бы ни говорили типы моста.
- Рабочую копию не трогай — она общая с автором и одна на машине: ни одной git-команды, пишущей в рабочее дерево, индекс или ссылки (`checkout`, `switch`, `restore`, `reset`, `stash`, `clean`, `apply`, `cherry-pick`, `worktree add` — примеры, не перечень); дифф и файлы ветки читай через `git diff origin/main...HEAD` и `git show <ref>:<путь>`. Порождающий даёт тебе отдельный worktree, где харнесс умеет, — и тогда запрет держит харнесс, а не ты.
