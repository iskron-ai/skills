...
- Прогони `integrity` read-only: для феноменов — trace в обе стороны; для крий — next-нить и trace `ahara`/`utpatti`/`upadhi`; выход в соседний холон — к его steward-роли. Прочти открытые вимарши на затронутых узлах.
- В `INTEGRATION` назови: затронутые холоны и роли; пройденные/оборванные эстафеты; открытые вопросы; статус соседей `ready|question|wake|graph-gap|unknown`; кого нужно разбудить через `collaborate`. `ready` не выводи из исходников — без достижимого свидетельства ставь `unknown`.
