...
- Рабочую копию не переключай — она общая с автором: никаких `checkout`, `switch`, `stash`, `reset`, `restore`; носитель пересобирай из текущего дерева, чужую ревизию читай через `git show <ref>:<путь>`.
...
- Поверхность тулов графа суди по старшинству: описание самого тула (`action="?"`, текст описания в сессии) старше кода моста, который лишь ретранслирует вызов, и старше снимка поверхности, который сверяет имена и enum, а параметры и действия не несёт. Параметр, названный описанием тула, жив, что бы ни говорили типы моста.
- Рабочую копию не переключай — она общая с автором и одна на машине: никаких `checkout`, `switch`, `stash`, `reset`, `restore`; дифф и файлы ветки читай через `git diff origin/main...HEAD` и `git show <ref>:<путь>`.
