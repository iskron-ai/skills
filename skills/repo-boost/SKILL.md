...
- Локальная директория проектной памяти (`~/.claude/projects/<encoded-path>/memory/`): проаудируй, **эвакуируй, затем заморозь**. Там не остаётся ничего — запрет по директории, не по категории. Проектное состояние (решения, ограничения, готчи) → AGENTS.md / NKS, состояние ветки → git и тело PR; долговременные пользовательские владения (машины, деплои, межпроектные находки) *и* предпочтения стиля работы, которые раньше пропускались, → личный граф пользователя (**minding**) — дом, переживающий новую машину и читаемый следующим агентом. Затем заморозь: `MEMORY.md` становится однострочным стабом-запретом («состояние живёт в репо или графе — см. AGENTS.md, Persistence rules»), а memory-guard (Шаг 4) блокирует дальнейшие записи в момент срабатывания инстинкта. **Эвакуируй прежде, чем заморозить** — стаб поверх непрочитанного уничтожает то, что должен был переселить. Причина: воспроизводимость + мультимашинная, мультиагентная работа — локальная память невидима любому другому агенту и машине.
...
- **Проштампуй контракт.** Хвостовая строка `AGENTS.md`: `*(repo-boost: contract <число из шапки этого файла> — re-run when the installed contract is higher, or when the sources this file derives from have moved since.)*` На рефреше перезапиши старый штамп; никогда не оставляй два. Конфиг **без** штампа — или со штампом-**датой** из схемы, которую этот счётчик сменил, — предшествует контракту: считай его ниже любого числа и прогони полную дугу.
