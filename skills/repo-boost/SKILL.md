...
Скелет сознательно *инлайнит* реалм-агностичную агентскую дисциплину (Рабочие принципы, части «Жизненного цикла сессии») в каждый сгенерированный `AGENTS.md`, чтобы файл стоял сам — для агентов без доступа к NKS. Держи её инлайн — не заменяй указателем в реалм методологии, хотя содержание там дублируется.
...
Не выбирай дефолты молча. Подтверди в разговоре и запиши в «Что это за проект»: **Nature** (и, если не `production`, какие принципы ослаблены и почему), **имя NKS-реалма** (создай, если нет), «Стек», **гейт качества** (предложи строжайший — Шаг 3). Для существующего репо сначала выведи из репо и старого конфига, подтверждай только двусмысленное.
...
- Если у проекта есть структура сверх самого реалма — существует фокус-холон (названный по границе проекта, `contains` от корня реалма), и его `#seq` идёт в «Что это за проект». Границу проектируй скиллом `design`, создавай `iskron_add_holon`.
