...
Также согласуй, **кто владеет направлением**: владелец графа как svatantra-карта (主) — создаётся в Шаге 2, если её нет. Вопросы вне мандата будут ставиться ей граф-вимаршами (`posed_to`), не только чатом.
...
- Если у проекта есть структура сверх самого графа — существует фокус-холон (названный по границе проекта, `contains` от корня графа), и его `#seq` идёт в «Что это за проект». Границу проектируй скиллом `design`, создавай `iskron_add_holon`.
- **Делатель становится стюардом.** Создай карту агента репозитория (`iskron_add_karta`, `manifested_as=adhikarin`, мотивация — дистиллят Production statement) и проведи её `steward` к фокус-холону: adhikarin без steward-ребра — живое предупреждение («действует, но ни за что не отвечает»). Создай svatantra-карту владельца, если Шаг 1 её не нашёл. Запиши оба seq в «Что это за проект». Агент всегда может пере-найти свои карты через `iskron_me(action="kartas")` — слот в доке — быстрый путь, не единственный.
...
- **`SessionStart`** → напоминание ориентироваться в NKS до действий (скилл `entry`), с именем *этого* графа, фокус-холона **и карты агента**: открыть повестку делателя (`iskron_orient(realm, focus="<seq карты агента>")`) — входящие `posed_to`-вимарши суть инбокс сессии; каждую взять или явно отложить.
...
Self-check: все три базовых хука на месте, и spec-write присутствует **тогда и только тогда**, когда interop-штамп AGENTS.md говорит `full` (нет штампа или `prose-only` → его НЕ должно быть — не до-добавляй на рефреше); `SessionStart` называет реальный слаг графа, фокус-холон и seq карты агента, не плейсхолдеры; ни одна чужая хук-запись не выпала при мерже.
