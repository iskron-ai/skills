...
Скилл **композирует** три других: `repo-boost` (понимание кодовой базы, реалм/холон), `intake` (источник-независимая дисциплина шабда-интейка — хребет Шага 5) и `assembly` (различение bianhua — сердце Шага 6). Сам он добавляет GitHub-**адаптер** (Шаги 3–4) и **рендер** роадмапа (Шаг 7). `references/self-checks.md` — полный проверочный гейт: Шаг 8 его прогоняет; без него не публикуй.
...
- Чтение в духе `repo-boost` настраивает реалм/холон, но никогда не пиши `AGENTS.md` в репо, владение которым только отыгрываешь.
...
- `repo-boost` — понимание кодовой базы + бутстрап реалма/холона.
