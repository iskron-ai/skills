...
Скилл **композирует** три других: `repo-boost` (понимание кодовой базы, граф/контур), `intake` (источник-независимая дисциплина шабда-интейка — хребет Шага 5) и `assembly` (различение превращений — сердце Шага 6). Сам он добавляет GitHub-**адаптер** (Шаги 3–4) и **рендер** роадмапа (Шаг 7). `references/self-checks.md` — полный проверочный гейт: Шаг 8 его прогоняет; без него не публикуй.
...
- Чтение в духе `repo-boost` настраивает граф/контур, но никогда не пиши `AGENTS.md` в репо, владение которым только отыгрываешь.
...
2. **`roadmap.md`**, в этом порядке:
   ...
   - **«Чтение поля»** — роли-драйверы (+ рантайм-цели), структурные риски, карта фигура-на-почве. Написано для мейнтейнера, не методолога: глоссируй каждый NKS-термин по-простому при первом употреблении.
...
- `repo-boost` — понимание кодовой базы + бутстрап графа/контура.
- `intake` — шабда-хребет Шага 5; Шаги 3–4 — его GitHub-адаптер.
- `assembly` — ритуал различения превращений (Шаг 6).
