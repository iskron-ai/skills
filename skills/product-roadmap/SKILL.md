...
Скилл **композирует** три других: `repo-boost` (понимание кодовой базы, граф/холон), `intake` (источник-независимая дисциплина шабда-интейка — хребет Шага 5) и `assembly` (различение bianhua — сердце Шага 6). Сам он добавляет GitHub-**адаптер** (Шаги 3–4) и **рендер** роадмапа (Шаг 7). `references/self-checks.md` — полный проверочный гейт: Шаг 8 его прогоняет; без него не публикуй.
...
- Сначала найди граф + фокус-холон (`iskron_realm` list; `iskron_orient` / `iskron_semantic_search`). Существует → ориентируйся и веди прогон как инкрементальное освежение. Отсутствует → предложи создание, создавай только по отмашке мейнтейнера.
...
- Чтение в духе `repo-boost` настраивает граф/холон, но никогда не пиши `AGENTS.md` в репо, владение которым только отыгрываешь.
...
- Не repo-boost, пишущий AGENTS.md в чужой репо.
...
- Не дупликатор графа — повторные прогоны освежают на месте; отсутствующие графы предлагаются.

## Скиллы-соседи

- `repo-boost` — понимание кодовой базы + бутстрап графа/холона.
