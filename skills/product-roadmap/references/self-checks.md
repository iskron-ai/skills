...
- [ ] **Ни одной роли с нулём рёбер; каждая роль несёт `manifested_as`.** У каждой созданной роли реальные рёбра `actor`/`steward` к деяниям, которые она двигает, — `iskron_look` на каждую. «Делатель»-воркер/CI/cron — ⚙️ феномен, не роль.
...
- [ ] Секция «чтение поля» на месте: роли-драйверы (+ рантайм-цели), структурные риски из натяжений, карта фигура-на-почве — каждый термин графа глоссирован по-простому при первом употреблении.
...
## Контракт повторного прогона (существующий граф)
...
- [ ] Граф отсутствовал → создание было *предложено* и одобрено, никогда молча.

## Дисциплина ссылок и языка

- [ ] `#N` только для реальных GitHub-номеров; seq графа с префиксом (`ISKRON#…`); направления — `D1`/`D2` или по имени.
...
- [ ] Мультирепо: ОДИН фокус-контур, каждый репо в объёме — подсистема под ним, и присутствует минимум одно кросс-репо-направление или кросс-репо-эстафета (иначе это N сшитых роадмапов).
