...
// УСТАРЕЛ, оставлен для вахт на нативном транспорте (без моста), поднятых до
// того, как сокет стал держать мост: обновление поставки роняло их молча
// (граф nks-dev: #4336). Путь вперёд — establish-mcp: мост держит сокет, а
// слушатель — подкоманда `watchdog` того же файла iskron.mjs, командой из блока
// [iskron-bridge] в ответе connect. Слово — в stderr: stdout здесь для кадра.
process.stderr.write(
  "watchdog: устаревший сторож нативного транспорта — переходи на мост (establish-mcp), " +
    "слушатель тогда — подкоманда watchdog файла iskron.mjs\n",
);
