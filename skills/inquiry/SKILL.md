...
| **hint** («будущий агент: прочти») | заведено строкой взятия → правится in-place всеми делателями превращения до состояния «что стоит сейчас» → закрыто координатором превращения на приёмке. Гроссбух превращения, одно семя на превращение: ничего из того, что покажут orient/линзы | воля: `visarjana` ∨ `virodha` |
