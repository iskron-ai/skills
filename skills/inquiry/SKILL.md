...
| **hint** («будущий агент: прочти») | прочитано → исполнено → закрыто. гроссбух превращения, одно семя на превращение: тело правится in-place до состояния «что стоит сейчас», ничего из того, что покажут orient/линзы; закрывает координатор превращения на приёмке, когда работа проросла в граф | воля: `visarjana` ∨ `virodha` |
