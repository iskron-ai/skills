...
|---|---|---|
| **Разрешение** | на вопрос отвечено; узел его закрывает | стрелка `addressed_by` (вимарша → разрешающий узел) + волевой `visarjana`. Если ответ — поведенческое утверждение («работает», «починено»), закрытию предшествует сверка реальностью с объявленной ступенью (дисциплина в on-duty): прозой поведенческий провал не закрывается |
