...
## Project structure
- `skills/<name>/SKILL.md` — **source of truth**, one dir per skill (`entry`, `writing`, `design`, `weaving`, `inquiry`, `assembly`, `integrity`, `intake`, `vahta`, `collaborate`, `foreman`, `feedback`, `reality-audit`, `minding`, `methodology-work`, `repo-boost`, `product-roadmap`, `establish-mcp`); `references/*` optional (`repo-boost`, `writing`, `product-roadmap`, `collaborate` ship them); `establish-mcp` ships `scripts/iskron-bridge.mjs` (the stdio↔https OAuth MCP bridge — code inside a skill, kept dependency-free, Node ≥20).
