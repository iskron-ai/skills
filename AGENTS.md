...
- **Версия моста датирует полевые репорты.** Поведенческое изменение `iskron-bridge.mjs` поднимает константу `VERSION` в том же коммите: каждая синтетическая ошибка и строка `grant.log` несут сборку `vX.Y.Z+хеш` (хеш собственного файла считается на старте и покрывает забытый бамп), и по цитате из репорта кастомера редакция кода читается без форензики по формулировкам.
