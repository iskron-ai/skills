...
- **Два деплоя, не путай их.** Состояние *этого проекта* живёт в графе `@nks/nks-dev`, и тулы там — `nks_*`; каждая сессия начинается с `nks_orient` в нём. `mcp.iskron.ru` с тулами `iskron_*` — это *продукт, который описывают скиллы*, и графа `nks-dev` там нет вовсе (проверено: на нём только `@cogito/experiments` и `@cogito/supervisor`). Отсюда правило: `iskron_*` — то, что пишут **скиллы**, их читатель работает против того сервера; `nks_*` — то, чем **ты** ведёшь состояние этого репо. Своей привязки к `@nks/nks-dev` у репо нет: `.mcp.json` биндит только `mcp.iskron.ru`, так что тулы графа приходят из пользовательского конфига харнесса — открытый вопрос об этом висит на #1506.
- **Фокус-контур**: `#1506 «📦 iskron/skills (скиллы агента для iskron)»`, под RU-деплоем `#1086`.
- **Роль агента**: `#931 «👨‍💻 Разработчик скилл-репозиториев агента»` — adhikarin, стюард #1506 (и соседнего скилл-репо #844). Твой инбокс: `nks_orient(realm="@nks/nks-dev", focus="931")` на старте сессии. Здесь не записан seq → репо не проходил iskronify; прогони его прежде, чем действовать.
...
- **Старт:** `nks_orient(realm="@nks/nks-dev", focus_holon="1506")`; ориентируйся по карте АКТИВНЫХ БЯНЬХУА (`lens="bianhua"` для леса) — открытая работа живёт anga-вимаршами на превращениях; затравка `genre=hint`, если она есть, — указатель на то, чего карта не несёт. Протокол исполняет скилл `entry`. Затем открой свою повестку: `nks_orient(realm="@nks/nks-dev", focus="931")` — входящие `posed_to`-вимарши и есть твой инбокс; подними или явно отложи каждую прежде, чем браться за работу в репо.
